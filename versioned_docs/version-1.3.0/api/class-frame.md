...
## frame.$(selector)
- `selector` <[string]> A selector to query frame for. See [working with selectors](api/working-with-selectors.md#working-with-selectors) for more details.
...
This method waits for an element matching `selector`, waits for [actionability](./actionability.md) checks, then scrolls the element into view if needed and uses [page.mouse](api/class-page.md#pagemouse) to click in the center of the element.
