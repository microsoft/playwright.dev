...
also fire for popup pages. See also [page.on('popup')](./class-page.md#pageonpopup) to receive events about popups relevant to a specific page.
...
> **NOTE** Use [page.waitForLoadState([state, options])](./class-page.md#pagewaitforloadstatestate-options) to wait until the page gets to a particular state (you should not need it in
most cases).
...
[page.addInitScript(script[, arg])](./class-page.md#pageaddinitscriptscript-arg) is not defined.
...
See [page.exposeBinding(name, playwrightBinding[, options])](./class-page.md#pageexposebindingname-playwrightbinding-options) for page-only version.
...
find them using [chromiumBrowserContext.backgroundPages()](./class-chromiumbrowsercontext.md#chromiumbrowsercontextbackgroundpages).
...
Page routes (set up with [page.route(url, handler)](./class-page.md#pagerouteurl-handler)) take precedence over browser context routes when request matches both
handlers.
...
> **NOTE** [page.setDefaultNavigationTimeout(timeout)](./class-page.md#pagesetdefaultnavigationtimeouttimeout) and [page.setDefaultTimeout(timeout)](./class-page.md#pagesetdefaulttimeouttimeout) take priority over
...
with page-specific extra HTTP headers set with [page.setExtraHTTPHeaders(headers)](./class-page.md#pagesetextrahttpheadersheaders). If page overrides a particular header,
