...
At every point of time, page exposes its current frame tree via the [page.mainFrame()](api/class-page.md#pagemainframe) and [frame.childFrames()](api/class-frame.md#framechildframes)
methods.
...
- [frame.addScriptTag(script)](api/class-frame.md#frameaddscripttagscript)
- [frame.addStyleTag(style)](api/class-frame.md#frameaddstyletagstyle)
...
`pageFunction`. See [Working with selectors](/api/working-with-selectors.md) for more details. If no elements match the
...
as a first argument to `pageFunction`. See [Working with selectors](/api/working-with-selectors.md) for more details.
