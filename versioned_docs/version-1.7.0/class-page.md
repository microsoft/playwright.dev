...
- [page.addScriptTag(script)](./class-page.md#pageaddscripttagscript)
- [page.addStyleTag(style)](./class-page.md#pageaddstyletagstyle)
...
- [page.exposeBinding(name, playwrightBinding[, options])](./class-page.md#pageexposebindingname-playwrightbinding-options)
...
to the dialog via [dialog.accept([promptText])](./class-dialog.md#dialogacceptprompttext) or [dialog.dismiss()](./class-dialog.md#dialogdismiss) methods.
...
respond to it via setting the input files using [fileChooser.setFiles(files[, options])](./class-filechooser.md#filechoosersetfilesfiles-options) that can be uploaded after that.
...
Emitted when the page opens a new tab or window. This event is emitted in addition to the [browserContext.on('page')](./class-browsercontext.md#browsercontextonpage),
...
[page.route(url, handler)](#pagerouteurl-handler) or [browserContext.route(url, handler)](./class-browsercontext.md#browsercontextrouteurl-handler).
...
## page.$(selector)
- `selector` <[string]> A selector to query for. See [working with selectors](./working-with-selectors.md) for more details.
...
> **NOTE** The order of evaluation of multiple scripts installed via [browserContext.addInitScript(script[, arg])](./class-browsercontext.md#browsercontextaddinitscriptscript-arg) and
...
Shortcut for main frame's [frame.addScriptTag(script)](./class-frame.md#frameaddscripttagscript).
...
Shortcut for main frame's [frame.addStyleTag(style)](./class-frame.md#frameaddstyletagstyle).
...
- `selector` <[string]> A selector to search for element. If there are multiple elements satisfying the selector, the first will be used. See [working with selectors](./working-with-selectors.md) for more details.
- `options` <[Object]>
...
[ChromiumCoverage](./class-chromiumcoverage.md#class-chromiumcoverage) for more details.
...
See [browserContext.exposeBinding(name, playwrightBinding[, options])](./class-browsercontext.md#browsercontextexposebindingname-playwrightbinding-options) for the context-wide version.
...
[browser.newContext([options])](./class-browser.md#browsernewcontextoptions) allows to set viewport size (and more) for all pages in the context at once.
