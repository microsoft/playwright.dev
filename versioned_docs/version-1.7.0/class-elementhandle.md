...
ElementHandle prevents DOM element from garbage collection unless the handle is disposed with [jsHandle.dispose()](./class-jshandle.md#jshandledispose).
ElementHandles are auto-disposed when their origin frame gets navigated.

ElementHandle instances can be used as an argument in [page.$eval(selector, pageFunction[, arg])](#pageevalselector-pagefunction-arg) and [page.evaluate(pageFunction[, arg])](./class-page.md#pageevaluatepagefunction-arg) methods.
...
argument to `pageFunction`. See [Working with selectors](./working-with-selectors.md) for more details. If no elements match
...
matched elements as a first argument to `pageFunction`. See [Working with selectors](./working-with-selectors.md) for more
details.
...
1. Wait for [actionability](./actionability.md) checks on the element, unless `force` option is set.
1. Scroll the element into view if needed.
1. Use [page.touchscreen](./class-page.md#pagetouchscreen) to tap in the center of the element, or the specified `position`.
1. Wait for initiated navigations to either succeed or fail, unless `noWaitAfter` option is set.
