...
Playwright can record videos for all pages in a [browser context](core-concepts.mdx#browser-contexts). Videos are saved upon context closure, so make sure to await `browserContext.close()`.
...
- [class: ConsoleMessage](./api/class-consolemessage.mdx#class-consolemessage)
...
- [event: 'pageerror'](./api/class-browsercontext.mdx#event-pageerror)
