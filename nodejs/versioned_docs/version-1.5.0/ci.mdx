...
1. **Ensure CI agent can run browsers**: Use [our Docker image](docker/README.mdx)
...
We have a [pre-built Docker image](docker/README.mdx) which can either be used directly, or as a reference to update your existing Docker definitions.
...
For Linux agents, you can use [our Docker container](docker/README.mdx) with Azure Pipelines support for [running containerized jobs](https://docs.microsoft.com/en-us/azure/devops/pipelines/process/container-phases?view=azure-devops). Alternatively, you can refer to the [Dockerfile](docker/README.mdx) to see additional dependencies that need to be installed on a Ubuntu agent.
...
We run our tests on CircleCI, with our [pre-built Docker image](docker/README.mdx). Running Playwright smoothly on CircleCI requires the following steps:

1. Use the pre-built [Docker image](docker/README.mdx) in your config like so:
...
Jenkins supports Docker agents for pipelines. Use the [Playwright Docker image](docker/README.mdx)
...
Bitbucket Pipelines can use public [Docker images as build environments](https://confluence.atlassian.com/bitbucket/use-docker-images-as-build-environments-792298897.html). To run Playwright tests on Bitbucket, use our public Docker image ([see Dockerfile](docker/README.mdx)).
...
To run Playwright tests on GitLab, use our public Docker image ([see Dockerfile](docker/README.mdx)).
...
binaries. This behavior can be [customized with environment variables](installation.mdx).
...
1. Set `PLAYWRIGHT_BROWSERS_PATH=0` as the environment variable before running
   ...
   directory and cache them with the package code. See [installation docs](installation.mdx).
...
On Linux agents, headful execution requires [Xvfb](https://en.wikipedia.org/wiki/Xvfb) to be installed. Our [Docker image](docker/README.mdx) and GitHub Action have Xvfb pre-installed. To run browsers in headful mode with Xvfb, add `xvfb-run` before the Node.js command.
