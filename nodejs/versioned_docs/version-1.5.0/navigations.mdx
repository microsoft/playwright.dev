...
- [`domcontentloaded`](api/class-page.mdx#event-domcontentloaded) event is fired
- page executes some scripts and loads resources like stylesheets and images
- [`load`](api/class-page.mdx#event-load) event is fired
...
### Wait for element
In lazy-loaded pages, it can be useful to wait until an element is visible with [`page.waitForSelector`](./api/class-page.mdx#pagewaitforselectorselector-options). Alternatively, page interactions like [`page.click`](./api/class-page.mdx#pageclickselector-options) auto-wait for elements.
...
### Custom wait
`page.click` can be combined with [`page.waitForLoadState`](./api/class-page.mdx#pagewaitforloadstatestate-options) to wait for a loading event.
...
Clicking an element could trigger asychronous processing before initiating the navigation. In these cases, it is recommended to explicitly call [`page.waitForNavigation`](api/class-page.mdx#pagewaitfornavigationoptions). For example:
...
Clicking an element could trigger multiple navigations. In these cases, it is recommended to explicitly [`page.waitForNavigation`](api/class-page.mdx#pagewaitfornavigationoptions) to a specific url. For example:
...
### Loading a popup
When popup is opened, explicitly calling [`page.waitForLoadState`](api/class-page.mdx#pagewaitforloadstatestate-options) ensures that popup is loaded to the desired state.
...
## Advanced patterns
For pages that have complicated loading patterns, [`page.waitForFunction`](api/class-page.mdx#pagewaitforfunctionpagefunction-arg-options) is a powerful and extensible approach to define a custom wait criteria.
