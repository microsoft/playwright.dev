...
Playwright Test creates an isolated [Page] object for each test. However, if you'd like to reuse a single [Page] object between multiple tests, you can create your own in [test.beforeAll()](/api/class-test.mdx#test-before-all-1) and close it in [test.afterAll()](/api/class-test.mdx#test-after-all-1).
