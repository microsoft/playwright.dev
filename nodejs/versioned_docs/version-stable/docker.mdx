...
```bash
docker pull mcr.microsoft.com/playwright:v1.46.1-jammy
```
...
```bash
docker run -it --rm --ipc=host --user pwuser --security-opt seccomp=seccomp_profile.json mcr.microsoft.com/playwright:v1.46.1-jammy /bin/bash
```
...
- `:v1.46.1` - Playwright v1.46.1 release docker image based on Ubuntu 22.04 LTS (Jammy Jellyfish).
