...
```bash
docker run -it --rm --ipc=host mcr.microsoft.com/playwright:v1.47.0-jammy /bin/bash
```
...
```bash
docker run -it --rm --ipc=host --user pwuser --security-opt seccomp=seccomp_profile.json mcr.microsoft.com/playwright:v1.47.0-jammy /bin/bash
```
