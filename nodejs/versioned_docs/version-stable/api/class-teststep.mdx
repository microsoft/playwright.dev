...
<font size="2" style={{position: "relative", top: "-20px"}}>Added in: v1.63</font><x-search>testStep.params</x-search>
...
### subtitle {/* #test-step-subtitle */}

<font size="2" style={{position: "relative", top: "-20px"}}>Added in: v1.63</font><x-search>testStep.subtitle</x-search>

User-friendly test step subtitle that complements the title, when available. For Playwright API calls, it is the target locator or the navigation url. For example, a `Click` step has the clicked locator as a subtitle. [test.step()](/api/class-test.mdx#test-step) steps carry the subtitle passed by the test author. User interfaces typically render the subtitle next to the title or on a separate line.
