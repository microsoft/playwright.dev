...
Called immediately before test runner exists. At this point all the reporters have recived the [reporter.onEnd()](/api/class-reporter.mdx#reporter-on-end) signal, so all the reports should be build. You can run the code that uploads the reports in this hook.
