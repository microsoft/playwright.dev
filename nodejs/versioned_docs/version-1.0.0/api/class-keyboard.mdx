...
Keyboard provides an api for managing a virtual keyboard. The high level api is [`keyboard.type`](api/class-keyboard.mdx#keyboardtypetext-options), which takes raw characters and generates proper keydown, keypress/input, and keyup events on your page.

For finer control, you can use [`keyboard.down`](api/class-keyboard.mdx#keyboarddownkey), [`keyboard.up`](api/class-keyboard.mdx#keyboardupkey), and [`keyboard.insertText`](api/class-keyboard.mdx#keyboardinserttexttext) to manually fire events as if they were generated from a real keyboard.
