...
We run our tests on CircleCI, with our [pre-built Docker image](./docker.mdx). Running Playwright smoothly on CircleCI requires the following steps:
...
On Linux agents, headful execution requires [Xvfb](https://en.wikipedia.org/wiki/Xvfb) to be installed. Our [Docker image](./docker.mdx) and GitHub Action have Xvfb pre-installed. To run browsers in headful mode with Xvfb, add `xvfb-run` before the Node.js command.
