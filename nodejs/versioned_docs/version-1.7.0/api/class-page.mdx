...
- [page.addScriptTag(script)](api/class-page.mdx#pageaddscripttagscript)
- [page.addStyleTag(style)](api/class-page.mdx#pageaddstyletagstyle)
...
to the dialog via [dialog.accept([promptText])](api/class-dialog.mdx#dialogacceptprompttext) or [dialog.dismiss()](api/class-dialog.mdx#dialogdismiss) methods.
...
respond to it via setting the input files using [fileChooser.setFiles(files[, options])](api/class-filechooser.mdx#filechoosersetfilesfiles-options) that can be uploaded after that.
...
Emitted when the page opens a new tab or window. This event is emitted in addition to the [browserContext.on('page')](api/class-browsercontext.mdx#browsercontextonpage),
...
[page.route(url, handler)](#pagerouteurl-handler) or [browserContext.route(url, handler)](api/class-browsercontext.mdx#browsercontextrouteurl-handler).
...
## page.$(selector)
- `selector` <[string]> A selector to query for. See [working with selectors](/api/working-with-selectors.mdx)) for more details.
...
> **NOTE** The order of evaluation of multiple scripts installed via [browserContext.addInitScript(script[, arg])](api/class-browsercontext.mdx#browsercontextaddinitscriptscript-arg) and
...
Shortcut for main frame's [frame.addScriptTag(script)](api/class-frame.mdx#frameaddscripttagscript).
...
Shortcut for main frame's [frame.addStyleTag(style)](api/class-frame.mdx#frameaddstyletagstyle).
...
- `selector` <[string]> A selector to search for element. If there are multiple elements satisfying the selector, the first will be used. See [working with selectors](/api/working-with-selectors.mdx) for more details.
- `options` <[Object]>
...
[ChromiumCoverage](api/class-chromiumcoverage.mdx#class-chromiumcoverage) for more details.
...
See [browserContext.exposeBinding(name, playwrightBinding[, options])](api/class-browsercontext.mdx#browsercontextexposebindingname-playwrightbinding-options) for the context-wide version.
...
This method waits for an element matching `selector`, waits for [actionability](./actionability.mdx) checks, focuses the
...
[browser.newContext([options])](api/class-browser.mdx#browsernewcontextoptions) allows to set viewport size (and more) for all pages in the context at once.
