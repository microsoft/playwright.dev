...
Whenever a network route is set up with [page.route(url, handler)](api/class-page.mdx#pagerouteurl-handler) or [browserContext.route(url, handler)](api/class-browsercontext.mdx#browsercontextrouteurl-handler), the `Route` object allows to
handle the route.
