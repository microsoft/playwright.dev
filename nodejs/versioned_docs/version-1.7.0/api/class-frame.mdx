...
At every point of time, page exposes its current frame tree via the [page.mainFrame()](api/class-page.mdx#pagemainframe) and [frame.childFrames()](api/class-frame.mdx#framechildframes)
methods.
...
- [frame.addScriptTag(script)](api/class-frame.mdx#frameaddscripttagscript)
- [frame.addStyleTag(style)](api/class-frame.mdx#frameaddstyletagstyle)
...
selectors](/api/working-with-selectors.mdx) for more details. If no elements match the selector, the return value resolves to
`null`.
...
`pageFunction`. See [Working with selectors](/api/working-with-selectors.mdx) for more details. If no elements match the
...
as a first argument to `pageFunction`. See [Working with selectors](/api/working-with-selectors.mdx) for more details.
