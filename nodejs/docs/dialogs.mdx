...
:::note
If your action, be it [page.click(selector[, options])](./api/class-page.mdx#pageclickselector-options), [page.evaluate(pageFunction[, arg])](./api/class-page.mdx#pageevaluatepagefunction-arg) or any other, results in a dialog, the action will stall until the dialog is handled. That's because dialogs in Web are modal and block further page execution until they are handled.
:::
