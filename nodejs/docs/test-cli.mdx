...
| `--last-run-file` | Path to the last-run file (default: "test-results/.last-run.json"). This json file is used for the `--last-failed` option. You can also apply a custom filter by providing an array of [testCase.id](/api/class-testcase.mdx#test-case-id) in the `filterTests` property. |
