...
| [queries](https://testing-library.com/docs/queries/about)                       | [locators](./locators.mdx)                                                 |
| [async helpers](https://testing-library.com/docs/dom-testing-library/api-async) | [assertions](./test-assertions.mdx)                                        |
...
1. Replace `render` with `mount` that returns a [component locator](./locators.mdx).
...
1. Use [assertions](./test-assertions.mdx) to verify the state.
...
Playwright includes [assertions](./test-assertions.mdx) that automatically wait for the condition, so you don't usually need an explicit `waitFor`/`waitForElementToBeRemoved` call.
...
- [Experimental Component Testing](./test-components.mdx)
