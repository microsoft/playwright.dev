...
1. Replace `render` with [fixtures.mount()](/api/class-fixtures.mdx#fixtures-mount), which takes a story id and returns a [component locator](./locators) scoped to the gallery root.
