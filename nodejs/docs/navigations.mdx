...
Clicking an element could trigger asychronous processing before initiating the navigation. In these cases, it is recommended to explicitly call [page.waitForNavigation([options])](./api/class-page.mdx#pagewaitfornavigationoptions). For example:
