...
Type into the field character by character, as if it was a user with a real keyboard with [locator.type()](/api/class-locator.mdx#locator-type).
...
:::note
Most of the time, [page.fill()](/api/class-page.mdx#page-fill) will just work. You only need to type characters if there is special keyboard handling on the page.
:::
