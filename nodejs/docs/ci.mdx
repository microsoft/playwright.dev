...
pool:
  vmImage: ubuntu-latest
container: mcr.microsoft.com/playwright:v1.46.1-noble
...
```groovy
pipeline {
   agent { docker { image 'mcr.microsoft.com/playwright:v1.46.1-noble' } }
   stages {
      stage('e2e-tests') {
         steps {
            sh 'npm ci'
...
```yml
steps:
- name: mcr.microsoft.com/playwright:v1.46.1-noble
  script: 
  ...
  env:
  - 'CI=true'
