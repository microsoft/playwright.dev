...
Always set a [global timeout](./test-timeouts.mdx#global-timeout) in CI. By default a test run has no upper bound, so a suite that hangs, or that slowly grows past the job limit of your CI provider, is killed by the runner mid-run and does not produce the test report.

With [testConfig.globalTimeout](/api/class-testconfig.mdx#test-config-global-timeout) set, Playwright stops the run itself.
...
```yml title=".github/workflows/playwright.yml" {23-25}
name: Playwright Tests
on:
  push:
    branches: [ main, master ]
...
```yml
steps:
- name: mcr.microsoft.com/playwright:v1.63.0-noble
  script:
  ...
  env:
  - 'CI=true'
