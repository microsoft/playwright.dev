...
1. Each Playwright Test file has explicit import of the `test` and `expect` functions 2. Test function is marked with `async` 3. Playwright Test is given a `page` as one of its parameters. This is one of the many [useful fixtures](./api/class-fixtures) in Playwright Test. 4. Almost all Playwright calls are prefixed with `await` 5. Locator creation with [page.locator(selector)](./api/class-page.mdx#page-locator) is one of the few methods that is sync.
...
1. Make sure you have protractor installed in your package.json 2. Polyfill function
