...
### globalScripts {#test-config-global-scripts}

<font size="2" style={{position: "relative", top: "-20px"}}>Added in: v1.30</font><x-search>testConfig.globalScripts</x-search>
...
[test.beforeAll()](/api/class-test.mdx#test-before-all) hooks in the matching files will run before testing starts. [test.afterAll()](/api/class-test.mdx#test-after-all) hooks in the matching files will run after testing finishes.

If global setup fails, test execution will be skipped. [test.afterAll()](/api/class-test.mdx#test-after-all) hooks will run in the same worker process as [test.beforeAll()](/api/class-test.mdx#test-before-all).
