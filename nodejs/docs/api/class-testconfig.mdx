...
- type: &#60;[Object]&#62;
  ...
  - `port` &#60;[number]&#62; Port to wait on for the web server
...
The port gets then passed over to Playwright as a `baseURL` when creating the context [browser.newContext([options])](./api/class-browser.mdx#browser-new-context). For example `8080` ends up in `baseURL` to be `http://localhost:8080`. If you want to use `https://` you need to manually specify the `baseURL` inside `use`.
