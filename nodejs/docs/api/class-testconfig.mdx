...
- [Object]
  ...
  - `toHaveScreenshot` [Object] *(optional)*
    ...
    - `stylePath` [string] | [Array]&lt;[string]&gt; *(optional)*
      ...
      See `style` in [page.screenshot()](/api/class-page.mdx#page-screenshot).
...
Whether to skip entries from `.gitignore` when searching for test files. By default, if neither [testConfig.testDir](/api/class-testconfig.mdx#test-config-test-dir) nor [testProject.testDir](/api/class-testproject.mdx#test-project-test-dir) are explicitely specified, Playwright will ignore any test files matching `.gitignore` entries. This option allows to override that behavior.
...
### shardingSeed {#test-config-sharding-seed}

<font size="2" style={{position: "relative", top: "-20px"}}>Added in: v1.45</font><x-search>testConfig.shardingSeed</x-search>
