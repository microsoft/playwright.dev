...
1. A builtin reporter name like `'list'` or `'json'`. 2. A module name like `'my-awesome-reporter'`. 3. A relative path to the reporter like `'./reporters/my-awesome-reporter.js'`.
...
The maximum number of retry attempts given to failed tests. Learn more about [test retries](./test-retries.mdx).
