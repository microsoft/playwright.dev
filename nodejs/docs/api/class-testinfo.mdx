...
Skips the currently running test. This is similar to [test.skip([condition, description])](./api/class-test.mdx#test-skip).
...
1. `'skipped'` for skipped tests, e.g. with [test.skip([condition, description])](./api/class-test.mdx#test-skip);
1. `'failed'` for tests marked as failed with [test.fail([condition, description])](./api/class-test.mdx#test-fail).
...
Specifies the retry number when the test is retried after a failure. The first test run has [testInfo.retry](./api/class-testinfo.mdx#test-info-retry) equal to zero, the first retry has it equal to one, and so on. Learn more about [retries](./test-retries.mdx).
