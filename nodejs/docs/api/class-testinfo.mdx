...
`TestInfo` contains information about currently running test. It is available to test functions, [test.beforeEach()](/api/class-test.mdx#test-before-each-1), [test.afterEach()](/api/class-test.mdx#test-after-each-1), [test.beforeAll()](/api/class-test.mdx#test-before-all-1) and [test.afterAll()](/api/class-test.mdx#test-after-all-1) hooks, and test-scoped fixtures. `TestInfo` provides utilities to control test execution: attach files, update test timeout, determine which test is currently running and whether it was retried, etc.
...
The number of milliseconds the test took to finish. Always zero before the test finishes, either successfully or not. Can be used in [test.afterEach()](/api/class-test.mdx#test-after-each-1) hook.
...
Actual status for the currently running test. Available after the test has finished in [test.afterEach()](/api/class-test.mdx#test-after-each-1) hook and fixtures.
