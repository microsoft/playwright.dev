...
- `options?` &#60;[Object]&#62;
  ...
  - `har?`<a aria-hidden="true" tabindex="-1" class="list-anchor-link" id="route-fulfill-option-har"/> &#60;[Object]&#62; HAR file to extract the response from. If HAR file contains an entry with the matching url and HTTP method, then the entry's headers, status and body will be used to fulfill. An entry resulting in a redirect will be followed automatically. Individual fields such as headers can be overridden using fulfill options. If `path` is a relative path, then it is resolved relative to the current working directory.<a href="#route-fulfill-option-har" class="list-anchor">#</a>
    - `path` &#60;[string]&#62; Path to the HAR file.
    - `fallback?` &#60;"abort"|"continue"|"throw"&#62; Behavior in the case where matching entry was not found in the HAR. Either [route.abort([errorCode])](./api/class-route.mdx#route-abort) the request, [route.continue([options])](./api/class-route.mdx#route-continue) it, or throw an error. Defaults to "abort".
