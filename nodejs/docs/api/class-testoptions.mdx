...
Name of the [TestStorage] entry that should be used to initialize [testOptions.storageState](/api/class-testoptions.mdx#test-options-storage-state). The value must be written to the test storage before creation of a browser context that uses it (usually in [testProject.setup](/api/class-testproject.mdx#test-project-setup)). If both this property and [testOptions.storageState](/api/class-testoptions.mdx#test-options-storage-state) are specified, this property will always take precedence.
