...
- [fixtures.actionTimeout](./api/class-fixtures.mdx#fixtures-action-timeout)
...
- [fixtures.createContext](./api/class-fixtures.mdx#fixtures-create-context)
...
- [fixtures.navigationTimeout](./api/class-fixtures.mdx#fixtures-navigation-timeout)
...
## fixtures.actionTimeout {#fixtures-action-timeout}
- type: &#60;[number]&#62;
...
## fixtures.createContext {#fixtures-create-context}
- type: &#60;[function]\([BrowserContextOptions]|[void]\):[BrowserContext]&#62;

A function that creates a new context, taking into account all options set through [configuration file](./test-configuration.mdx) or [test.use(fixtures)](./api/class-test.mdx#test-use) calls. All contexts created by this function are similar to the default [fixtures.context](./api/class-fixtures.mdx#fixtures-context).

This function is useful for multi-context scenarios, for example testing two users talking over the chat application.

A single `options` argument will be merged with all the default options from [configuration file](./test-configuration.mdx) or [test.use(fixtures)](./api/class-test.mdx#test-use) calls and passed to [browser.newContext([options])](./api/class-browser.mdx#browser-new-context). If you'd like to undo some of these options, override them with some value or `undefined`. For example:
...
## fixtures.navigationTimeout {#fixtures-navigation-timeout}
- type: &#60;[number]&#62;
