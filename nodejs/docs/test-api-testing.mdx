...
The main difference is that [APIRequestConxtext] accessible via [browserContext.request](./api/class-browsercontext.mdx#browser-context-request) and [page.request](./api/class-page.mdx#page-request) will populate request's `Cookie` header from the browser context and will automatically update browser cookies if [APIResponse] has `Set-Cookie` header:
