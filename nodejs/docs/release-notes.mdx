...
- New web-first assertions for screenshots: [expect(page).toHaveScreenshot([options])](./api/class-playwrightassertions.mdx#page-assertions-to-have-screenshot) and [expect(locator).toHaveScreenshot([options])](./api/class-playwrightassertions.mdx#locator-assertions-to-have-screenshot). These methods will re-take screenshot until it matches the saved expectation. When generating a new expectation, the method will re-take screenshots until 2 consecutive screenshots match.
