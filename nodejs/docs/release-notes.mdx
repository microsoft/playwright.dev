...
* UI mode and trace viewer mark network requests handled with [page.route()](/api/class-page.mdx#page-route) and [browserContext.route()](/api/class-browsercontext.mdx#browser-context-route) handlers, as well as those issued via the [API testing](./api-testing.mdx):
...
Read more about [component testing with Playwright](./test-components.mdx).
...
- Components Testing (preview)
  ...
  Read more in [our documentation](./test-components.mdx).
...
- [`expect(response).toBeOK()`](./test-assertions.mdx)
...
It is now possible to do response interception by combining [API Testing](./api-testing.mdx) with [request interception](./network#modify-requests).
...
Learn more in the [documentation](./test-webserver.mdx).
