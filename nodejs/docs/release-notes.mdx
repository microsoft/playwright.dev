...
- `noWaitAfter` option in [locator.selectOption()](/api/class-locator.mdx#locator-select-option) was deprecated.
...
### New APIs
- Options `reason` in [page.close()](/api/class-page.mdx#page-close), `reason` in [browserContext.close()](/api/class-browsercontext.mdx#browser-context-close) and `reason` in [browser.close()](/api/class-browser.mdx#browser-close). Close reason is reported for all operations interrupted by the closure.
...
- New option `maxRedirects` for method [route.fetch()](/api/class-route.mdx#route-fetch).
