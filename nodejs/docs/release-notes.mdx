...
- [page.getByRole(role[, options])](/api/class-page.mdx#page-get-by-role) to locate by [ARIA role](https://www.w3.org/TR/wai-aria-1.2/#roles), [ARIA attributes](https://www.w3.org/TR/wai-aria-1.2/#aria-attributes) and [accessible name](https://w3c.github.io/accname/#dfn-accessible-name).
...
### Behavior Changes
- [expect(locator).toHaveAttribute(name, value[, options])](/test-assertions.mdx#locator-assertions-to-have-attribute) with an empty value does not match missing attribute anymore. For example, the following snippet will succeed when `button` **does not** have a `disabled` attribute.
...
- New option `visible` for [expect(locator).toBeVisible([options])](/test-assertions.mdx#locator-assertions-to-be-visible).

### Other highlights
- New option `maxRedirects` for [apiRequestContext.get(url[, options])](/api/class-apirequestcontext.mdx#api-request-context-get) and others to limit redirect count.
...
### Test Runner
* [test.step(title, body)](/api/class-test.mdx#test-step) now returns the value of the step function:
...
* Added [test.describe.fixme(title, callback)](/api/class-test.mdx#test-describe-fixme).
...
It is now possible to call [test.describe(callback)](/api/class-test.mdx#test-describe-2) to create suites without a title. This is useful for giving a group of tests a common option with [test.use(options)](/api/class-test.mdx#test-use).
...
Use the new methods [page.routeFromHAR(har[, options])](/api/class-page.mdx#page-route-from-har) or [browserContext.routeFromHAR(har[, options])](/api/class-browsercontext.mdx#browser-context-route-from-har) to serve matching responses from the [HAR](http://www.softwareishard.com/blog/har-12-spec/) file:
...
You can now use [route.fallback([options])](/api/class-route.mdx#route-fallback) to defer routing to other handlers.
...
Note that the new methods [page.routeFromHAR(har[, options])](/api/class-page.mdx#page-route-from-har) and [browserContext.routeFromHAR(har[, options])](/api/class-browsercontext.mdx#browser-context-route-from-har) also participate in routing and could be deferred to.

### Web-First Assertions Update
* New method [expect(locator).toHaveValues(values[, options])](/test-assertions.mdx#locator-assertions-to-have-values) that asserts all selected values of `<select multiple>` element.
* Methods [expect(locator).toContainText(expected[, options])](/test-assertions.mdx#locator-assertions-to-contain-text) and [expect(locator).toHaveText(expected[, options])](/test-assertions.mdx#locator-assertions-to-have-text) now accept `ignoreCase` option.
...
- New [locator.filter([options])](/api/class-locator.mdx#locator-filter) API to filter an existing locator
...
- New web-first assertions [expect(page).toHaveScreenshot(name[, options])](/test-assertions.mdx#page-assertions-to-have-screenshot-1) and [expect(locator).toHaveScreenshot(name[, options])](/test-assertions.mdx#locator-assertions-to-have-screenshot-1) that wait for screenshot stabilization and enhances test reliability.
  ...
  * uses CSS scale option
  ...
  The new [expect(page).toHaveScreenshot(name[, options])](/test-assertions.mdx#page-assertions-to-have-screenshot-1) saves screenshots at the same location as [expect(screenshot).toMatchSnapshot(name[, options])](/test-assertions.mdx#screenshot-assertions-to-match-snapshot-1).
...
- New role selectors that allow selecting elements by their [ARIA role](https://www.w3.org/TR/wai-aria-1.2/#roles), [ARIA attributes](https://www.w3.org/TR/wai-aria-1.2/#aria-attributes) and [accessible name](https://w3c.github.io/accname/#dfn-accessible-name).
  ...
  Read more in [our documentation](./selectors#role-selector).
- New `scale` option in [page.screenshot([options])](/api/class-page.mdx#page-screenshot) for smaller sized screenshots.
- New `caret` option in [page.screenshot([options])](/api/class-page.mdx#page-screenshot) to control text caret. Defaults to `"hide"`.
...
- Playwright now supports large file uploads (100s of MBs) via [locator.setInputFiles(files[, options])](/api/class-locator.mdx#locator-set-input-files) API.
