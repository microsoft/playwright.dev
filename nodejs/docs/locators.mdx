...
- [page.getByTestId()](#locate-by-testid) to locate an element based on its `data-testid` attribute (other attributes can be configured).
...
Use [expect(locator).toHaveText()](/test-assertions.mdx#locator-assertions-to-have-text) to ensure that the list has the text "apple", "banana" and "orange".
