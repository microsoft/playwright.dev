...
- [page.getByRole(role[, options])](#locate-based-on-accessible-attributes) to locate by explicit and implicit accessibility attributes.
- [page.getByText(text[, options])](#locate-by-text) to locate by text content.
- [page.getByLabel(text[, options])](#locate-by-label-text) to locate a form control by associated label's text.
- [page.getByPlaceholder(text[, options])](#locate-by-placeholder-text) to locate an input by placeholder.
- [page.getByAltText(text[, options])](#locate-by-alt-text) to locate an element, usually image, by its text alternative.
- [page.getByTitle(text[, options])](#locate-by-title) to locate an element by its title.
- [page.getByTestId(testId)](#define-explicit-contract-and-use-a-data-testid-attribute) to locate an element based on its `data-testid` attribute (other attribute can be configured).
