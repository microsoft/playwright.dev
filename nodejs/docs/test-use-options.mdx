...
| Option | Description |
| :- | :- |
| [testOptions.colorScheme](/api/class-testoptions.mdx#test-options-color-scheme) | [Emulates](./emulation.mdx#color-scheme-and-media) `'prefers-colors-scheme'` media feature, supported values are `'light'`, `'dark'`, `'no-preference'` |
