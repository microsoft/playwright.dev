...
This guide is for anyone who wants to use Playwright with the VS Code extension. If you prefer to install using the CLI then please see the [Getting Started (CLI)](./getting-started-cli.mdx) docs.
...
At the moment we are just running our tests locally but you can run your tests on CI on each pull request thanks to the GitHub actions we setup earlier. Tests can be run on a local dev environment or on a staging URL. Checkout our guide for more options on [CI Configurations](./ci.mdx)
