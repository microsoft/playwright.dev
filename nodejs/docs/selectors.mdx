...
Selectors are strings that are used to create [Locator]s. Locators are used to perform actions on the elements by means of methods such as [locator.click()](/api/class-locator.mdx#locator-click), [locator.fill()](/api/class-locator.mdx#locator-fill) and alike. For debugging selectors, see [here](./debug-selectors).
...
You can add filtering to any locator by passing `:scope` selector to [locator.locator()](/api/class-locator.mdx#locator-locator) and specifying desired options. For example, given the locator `row` that selects some rows in the table, you can filter to just those that contain text "Hello".
