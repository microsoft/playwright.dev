...
Selector describes an element in the page. It can be used to obtain `ElementHandle` (see [page.$(selector)](./api/class-page.mdx#pageselector) for example) or shortcut element operations to avoid intermediate handle (see [page.click(selector[, options])](./api/class-page.mdx#pageclickselector-options) for example).
