...
Since Playwright runs in Node.js, you can debug it with your debugger of choice e.g. inside your IDE. Playwright comes with the Playwright Inspector which allows you to step through Playwright API calls, see their debug logs and explore [selectors](./selectors.mdx). 
...
We recommend using `--debug` to open the Playwright Inspector and debug your tests. However there are other ways including with [Browser Developer Tools](./debug.mdx#browser-developer-tools), [PWDEBUG](./debug.mdx#PWDEBUG), and [page.pause](./debug.mdx#page.pause)
- Debugging all tests:
