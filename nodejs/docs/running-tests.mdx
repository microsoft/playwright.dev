...
Playwright stores the list of failed tests from the previous run in `<outputDir>/.last-run.json` (see [testConfig.outputDir](./test-configuration.mdx)). To use a different file path, pass `--last-failed-file=<path>` or set `PLAYWRIGHT_LAST_RUN_OUTPUT_FILE`.
