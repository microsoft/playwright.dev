...
Since Playwright runs in Node.js, you can debug it with your debugger of choice e.g. using `console.log` or inside your IDE or directly in VS Code with the [VS Code Extension](./getting-started-vscode.mdx). Playwright comes with the [Playwright Inspector](./debug.mdx#playwright-inspector) which allows you to step through Playwright API calls, see their debug logs and explore [selectors](./locators.mdx#selectors).
- Debugging all tests:
