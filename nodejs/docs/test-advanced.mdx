...
You can also have project-specific setup with [testProject.projectSetup](./api/class-testproject.mdx#test-project-project-setup). It will only be executed if at least one test from a specific project should be run, while global setup is always executed at the start of the test session.
