...
Under most circumstances, for end-to-end testing, you'll want to use `@playwright/test` (Playwright Test), and not `playwright` (Playwright Library) directly. To get started with Playwright Test, follow its [Getting Started Guide](./intro.mdx).

- [When Should Playwright Library Be Used Directly?](#when-should-playwright-library-be-used-directly)
...
- creating an integration for a third party test runner (e.g. the third-party runner plugins listed [here](./test-runners.mdx) are built on top of Playwright Library)
...
| Initialization | Explicitly need to: <ol><li>Pick a browser to use (e.g. `chromium`)</li><li>Create `browser` ([browserType.launch([options])](/api/class-browsertype.mdx#browser-type-launch))</li><li>Create a `context` ([browser.newContext([options])](/api/class-browser.mdx#browser-new-context)), <em>and</em> pass any context options explcitly (e.g. `devices['iPhone 11']`</li><li>Create a `page` ([browserContext.newPage()](/api/class-browsercontext.mdx#browser-context-new-page))</li></ol> | An isolated `page` and `context` are provided to each test out-of the box (along with any other [built-in fixtures](./test-fixtures.mdx#built-in-fixtures)). No explicit creation. If referenced by the test in it's arguments, the Test Runner will create them for the test. (i.e. lazy-initialization) |
...
| Cleanup | Explicitly need to: <ol><li>Close `context`  ([browserContext.close()](/api/class-browsercontext.mdx#browser-context-close))</li><li>Close `browser`  ([browser.close()](/api/class-browser.mdx#browser-close))</li></ol> | No explicit close of [built-in fixtures](./test-fixtures.mdx#built-in-fixtures); the Test Runner will take care of it.
