...
* **Resilient element selectors**. Playwright can rely on user-facing strings, like text content and accessibility labels to [select elements](./locators.mdx#selectors). These strings are more resilient than selectors tightly-coupled to the DOM structure.
...
* **Modern web features**. Playwright supports web components through [shadow-piercing selectors](./locators.mdx#selectors), [geolocation, permissions](./emulation.mdx), web workers and other modern web APIs.
