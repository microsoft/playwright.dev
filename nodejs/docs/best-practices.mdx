...
You can also reuse the signed-in state in the tests with [global setup](/auth.mdx#reuse-signed-in-state). That way you can log in only once and then skip the log in step for all of the tests.
