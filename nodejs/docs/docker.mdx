...
```bash
docker run -it --rm --ipc=host --user pwuser --security-opt seccomp=seccomp_profile.json mcr.microsoft.com/playwright:v1.43.1-jammy /bin/bash
```
...
- `:v1.43.1` - Playwright v1.43.1 release docker image based on Ubuntu 22.04 LTS (Jammy Jellyfish).
- `:v1.43.1-jammy` - Playwright v1.43.1 release docker image based on Ubuntu 22.04 LTS (Jammy Jellyfish).
- `:v1.43.1-focal` - Playwright v1.43.1 release docker image based on Ubuntu 20.04 LTS (Focal Fossa).
