...
```bash
docker pull mcr.microsoft.com/playwright:v1.46.0-noble
```
...
```bash
docker run -it --rm --ipc=host mcr.microsoft.com/playwright:v1.46.0-noble /bin/bash
```
...
```bash
docker run -it --rm --ipc=host --user pwuser --security-opt seccomp=seccomp_profile.json mcr.microsoft.com/playwright:v1.46.0-noble /bin/bash
```
