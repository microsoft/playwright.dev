...
With [test.use(options)](/api/class-test.mdx#test-use) you can override some options for a file or a [test.describe(title, callback)](/api/class-test.mdx#test-describe-1) block.
...
- `storageState` - Populates context with given storage state. Useful for easy authentication, [learn more](./test-auth.mdx).
