...
Routes sit between the page and the browser's network stack. The handler runs before the network stack has processed the request: [route.continue()](/api/class-route.mdx#route-continue) passes it on, [route.fulfill()](/api/class-route.mdx#route-fulfill) answers it without touching the network, and [route.abort()](/api/class-route.mdx#route-abort) fails it.
