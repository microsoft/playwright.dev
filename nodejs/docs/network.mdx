...
Alternatively, instead of using the CLI, you can record HAR programmatically. Pass `har` option when creating a [BrowserContext] with [browser.newContext([options])](./api/class-browser.mdx#browser-new-context) to create an archive.
...
Pass `har` option to the [browser.newContext([options])](./api/class-browser.mdx#browser-new-context) method to use matching responses from the HAR file.
