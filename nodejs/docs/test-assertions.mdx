...
- [expect(pageOrLocator).toHaveScreenshot([options])](#expectpageorlocatortohavescreenshotoptions)
...
- `options`
  - `threshold` <[float]> an acceptable percieved color difference in the [YIQ color space](https://en.wikipedia.org/wiki/YIQ) between pixels in compared images, between zero (strict) and one (lax), default is configurable with [testConfig.expect](./api/class-testconfig.mdx#test-config-expect). Defaults to `0.2`.
...
- `options`
  ...
  - `mask` <[Array]<[Locator]>> Specify locators that should be masked when the screenshot is taken. Masked elements will be overlayed with a pink box `#FF00FF` that completely covers its bounding box.
