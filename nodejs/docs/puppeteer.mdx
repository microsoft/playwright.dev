...
This guide describes migration to [Playwright Library](./library.mdx) and [Playwright Test](./intro.mdx) from Puppeteer. The APIs have similarities, but Playwright offers much more possibilities for web testing and cross-browser automation.
...
| `await page.$eval(...)`                            | [Assertions](./test-assertions.mdx) can often be used instead to verify text, attribute, class... |
...
`page.waitForNavigation` and `page.waitForSelector` remain, but in many cases will not be necessary due to [auto-waiting](./actionability.mdx).
...
1. Use [assertions](./test-assertions.mdx) to verify the state instead of `page.$eval()`.

## Testing

To improve testing, it is advised to use [Locators](./api/class-locator) and web-first [Assertions](./test-assertions.mdx). See [Writing Tests](./writing-tests.mdx)

It is common with Puppeteer to use `page.evaluate()` or `page.$eval()` to inspect an [ElementHandle] and extract the value of text content, attribute, class... Web-first [Assertions](./test-assertions.mdx) offers several matchers for this purpose, it is more reliable and readable.
...
- Full support for multiple origins, [(i)frames](./api/class-frame), [tabs and contexts](./pages.mdx)
...
- [Fixtures](./test-fixtures.mdx)
- [Locators](./locators.mdx)
- [Assertions](./test-assertions.mdx)
- [Auto-waiting](./actionability.mdx)
