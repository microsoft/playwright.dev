...
- [Conditionally skip a test](#conditionally-skip-a-test)
- [Group tests](#group-tests)
- [Tag tests](#tag-tests)
- [Conditionally skip a group of tests](#conditionally-skip-a-group-of-tests)
- [Use fixme in `beforeEach` hook](#use-fixme-in-beforeeach-hook)
...
- [test.fail([condition, description])](./api/class-test.mdx#test-fail) marks the test as failing. Playwright Test will run this test and ensure it does indeed fail. If the test does not fail, Playwright Test will complain.
- [test.fixme(title, testFunction)](./api/class-test.mdx#test-fixme-1) marks the test as failing. Playwright Test will not run this test, as opposite to the `fail` annotation. Use `fixme` when running the test is slow or crashy.
- [test.slow([condition, description])](./api/class-test.mdx#test-slow) marks the test as slow and triples the test timeout.
