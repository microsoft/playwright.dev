...
You can also filter tests in the configuration file via [testConfig.tagFilter](/api/class-testconfig.mdx#test-config-tag-filter) and [testProject.tagFilter](/api/class-testproject.mdx#test-project-tag-filter).
