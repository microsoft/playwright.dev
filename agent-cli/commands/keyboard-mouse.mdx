...
| `type <text>` | Type text key by key into the focused element |
...
`keydown` / `keyup` hold a modifier across several commands, which `press` cannot do:

```bash
playwright-cli keydown Shift
playwright-cli click e5
playwright-cli click e9                 # extends the selection
playwright-cli keyup Shift
```

To hold modifiers for a single click, `click --modifiers` is simpler — see
...
Coordinates are viewport-relative CSS pixels. `snapshot --boxes` reports each element's box in the
same coordinate space, which beats reading positions off a screenshot.
