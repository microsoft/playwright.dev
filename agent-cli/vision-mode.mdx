...
| `snapshot --boxes` | Snapshot with each element's bounding box |

Coordinates are viewport-relative CSS pixels. `screenshot --hires` captures device pixels instead,
which accounts for the device pixel ratio — remember that its pixel coordinates are then *not* the
ones the mouse commands expect.

## Getting coordinates without guessing

When an element *is* in the accessibility tree but you need its position — to drag it, or to draw a
callout over it — `snapshot --boxes` is more reliable than reading a screenshot:
