...
Selectors query elements on the web page for interactions, like [page.click(selector[, options])](./class-page.md#pageclickselector-options), and to obtain `ElementHandle` through [page.$(selector)](./class-page.md#pageselector). Built-in selectors auto-pierce [shadow DOM](https://developer.mozilla.org/en-US/docs/Web/Web_Components/Using_shadow_DOM).
...
Selector describes an element in the page. It can be used to obtain `ElementHandle` (see [page.$(selector)](./class-page.md#pageselector) for example) or shortcut element operations to avoid intermediate handle (see [page.click(selector[, options])](./class-page.md#pageclickselector-options) for example).
