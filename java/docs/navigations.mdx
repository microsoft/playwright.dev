...
:::note
If the page does a client-side redirect before `load`, [Page.navigate()](/api/class-page.mdx#page-goto) will wait for the redirected page to fire the `load` event.
:::
...
At some point in time, you'll stumble upon a use case where Playwright performs an action, but nothing seemingly happens. Or you enter some text into the input field and will disappear. The most probable reason behind that is a poor page [hydration](https://en.wikipedia.org/wiki/Hydration_(web_development)).
