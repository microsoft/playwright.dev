...
- Pattern 3: resolve locator to elements on page and map them to their text content. Note that code inside [Locator.evaluateAll(expression[, arg])](/api/class-locator.mdx#locator-evaluate-all) runs in the web page and you can call any DOM apis there.
