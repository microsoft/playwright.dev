...
If you would like to use codegen in some non-standard setup (for example, use [BrowserContext.route(url, handler)](./api/class-browsercontext.mdx#browser-context-route)), it is possible to call [Page.pause()](./api/class-page.mdx#page-pause) that will open a separate window with codegen controls.
