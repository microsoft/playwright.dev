...
jobs:
  playwright:
    ...
    runs-on: ubuntu-latest
    container:
      image: mcr.microsoft.com/playwright/java:v1.36.0-jammy
    steps:
      - uses: actions/checkout@v3
      - uses: actions/setup-java@v3
        with:
...
pool:
  vmImage: ubuntu-latest
container: mcr.microsoft.com/playwright/java:v1.36.0-jammy
...
```groovy
pipeline {
   agent { docker { image 'mcr.microsoft.com/playwright/java:v1.36.0-jammy' } }
   stages {
      stage('e2e-tests') {
         steps {
