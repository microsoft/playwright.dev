...
- `cookies` <[List]<`Cookie`>>
  - `withName` <[String]>
  - `withValue` <[String]>
  - `withUrl` <[String]> either url or domain / path are required. Optional.
  - `withDomain` <[String]> either url or domain / path are required Optional.
  - `withPath` <[String]> either url or domain / path are required Optional.
  - `withExpires` <[double]> Unix time in seconds. Optional.
  - `withHttpOnly` <[boolean]> Optional.
  - `withSecure` <[boolean]> Optional.
  - `withSameSite` <`enum SameSiteAttribute { STRICT, LAX, NONE }`> Optional.
...
- `options` <`BrowserContext.GrantPermissionsOptions`>
  - `withOrigin` <[String]> The [origin] to grant permissions to, e.g. "https://example.com".
...
- `options` <`BrowserContext.StorageStateOptions`>
  - `withPath` <[Path]> The file path to save the storage state to. If `path` is a relative path, then it is resolved relative to current working directory. If no path is provided, storage state is still returned, but won't be saved to the disk.
- returns: <[String]>

Returns storage state for this browser context, contains current cookies and local storage snapshot.

## BrowserContext.unroute(url[, handler])
- `url` <[String]|[Pattern]|[function]\([String]\)> A glob pattern, regex pattern or predicate receiving [URL] used to register a routing with [BrowserContext.route(url, handler)](./api/class-browsercontext.mdx#browsercontextrouteurl-handler).
