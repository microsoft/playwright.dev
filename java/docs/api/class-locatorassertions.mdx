...
Ensures the [Locator] points to an element with given CSS classes. When a string is provided, it must fully match the element's `class` attribute. To match individual classes or perform partial matches use [setPartial](/api/class-locatorassertions.mdx#locator-assertions-to-have-class-option-partial).
...
- `options` `LocatorAssertions.HasClassOptions` *(optional)*
  - `setPartial` [boolean] *(optional)* <font size="2">Added in: v1.52</font><a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="locator-assertions-to-have-class-option-partial"/><a href="#locator-assertions-to-have-class-option-partial" class="list-anchor">#</a>
