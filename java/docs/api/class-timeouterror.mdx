...
TimeoutError is emitted whenever certain operations are terminated due to timeout, e.g. [Page.waitForSelector(selector[, options])](./api/class-page.mdx#pagewaitforselectorselector-options) or [BrowserType.launch([options])](./api/class-browsertype.mdx#browsertypelaunchoptions).
