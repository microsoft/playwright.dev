...
- [BrowserType.connectOverCDP([options])](./api/class-browsertype.mdx#browsertypeconnectovercdpoptions)
...
## BrowserType.connectOverCDP([options])
...
- `options` <`BrowserType.LaunchPersistentContextOptions`>
  ...
  - `setChannel` <`enum BrowserChannel { CHROME, CHROME_BETA, CHROME_DEV, CHROME_CANARY, MSEDGE, MSEDGE_BETA, MSEDGE_DEV, MSEDGE_CANARY }`> Browser distribution channel.
