...
- [Tracing.start([options])](./api/class-tracing.mdx#tracingstartoptions)
- [Tracing.stop()](./api/class-tracing.mdx#tracingstop)

## Tracing.export(path)
- `path` <[Path]> File to save the trace into.
- returns: <[void]>
...
- `options` <`Tracing.StartOptions`>
  - `setName` <[String]> If specified, the trace is going to be saved into the file with the given name inside the `traceDir` folder specified in [BrowserType.launch([options])](./api/class-browsertype.mdx#browsertypelaunchoptions).
