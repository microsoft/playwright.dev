...
- [Tracing.startChunk()](./api/class-tracing.mdx#tracing-start-chunk)
...
## Tracing.startChunk() {#tracing-start-chunk}
...
Start a new trace chunk. If you'd like to record multiple traces on the same [BrowserContext], use [Tracing.start([options])](./api/class-tracing.mdx#tracing-start) once, and then create multiple trace chunks with [Tracing.startChunk()](./api/class-tracing.mdx#tracing-start-chunk) and [Tracing.stopChunk([options])](./api/class-tracing.mdx#tracing-stop-chunk).
...
- `options` &#60;`Tracing.StopChunkOptions`&#62;
  - `setPath`<a aria-hidden="true" tabindex="-1" class="list-anchor-link" id="tracing-stop-chunk-option-path"/> &#60;[Path]&#62; Export trace collected since the last [Tracing.startChunk()](./api/class-tracing.mdx#tracing-start-chunk) call into the file with the given path.<a href="#tracing-stop-chunk-option-path" class="list-anchor">#</a>
