...
- `options` \<`Browser.NewContextOptions`\>
  ...
  - `setStorageState` \<[String]\> Populates context with given storage state. This option can be used to initialize context with logged-in information obtained via [BrowserContext.storageState([options])](./api/class-browsercontext.mdx#browser-context-storage-state).
  - `setStorageStatePath` \<[Path]\> Populates context with given storage state. This option can be used to initialize context with logged-in information obtained via [BrowserContext.storageState([options])](./api/class-browsercontext.mdx#browser-context-storage-state). Path to the file with saved storage state.
