...
- `options` &#60;`Route.FulfillOptions`&#62;
  ...
  - `setHar`<a aria-hidden="true" tabindex="-1" class="list-anchor-link" id="route-fulfill-option-har"/> &#60;[Path]&#62; HAR file to extract the response from. If HAR file contains an entry with the matching the url, its headers, status and body will be used. Individual fields such as headers can be overridden using fulfill options. If matching entry is not found, this method will throw. If `har` is a relative path, then it is resolved relative to the current working directory.<a href="#route-fulfill-option-har" class="list-anchor">#</a>
