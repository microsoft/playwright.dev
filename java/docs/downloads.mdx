...
For every attachment downloaded by the page, [Page.onDownload(handler)](./api/class-page.mdx#page-event-download) event is emitted. If you create a browser context with the `acceptDownloads` set, all these attachments are going to be downloaded into a temporary folder. You can obtain the download url, file system path and payload stream using the [Download] object from the event.
