...
### Behavior Changes
- [assertThat(locator).hasAttribute()](/test-assertions.mdx#locator-assertions-to-have-attribute) with an empty value does not match missing attribute anymore. For example, the following snippet will succeed when `button` **does not** have a `disabled` attribute.
...
### Assertions
- New option `enabled` for [assertThat(locator).isEnabled()](/test-assertions.mdx#locator-assertions-to-be-enabled).
- [assertThat(locator).hasText()](/test-assertions.mdx#locator-assertions-to-have-text) now pierces open shadow roots.
- New option `editable` for [assertThat(locator).isEditable()](/test-assertions.mdx#locator-assertions-to-be-editable).
- New option `visible` for [assertThat(locator).isVisible()](/test-assertions.mdx#locator-assertions-to-be-visible).
...
* New method [assertThat(locator).hasValues()](/test-assertions.mdx#locator-assertions-to-have-values) that asserts all selected values of `<select multiple>` element.
* Methods [assertThat(locator).containsText()](/test-assertions.mdx#locator-assertions-to-contain-text) and [assertThat(locator).hasText()](/test-assertions.mdx#locator-assertions-to-have-text) now accept `ignoreCase` option.
