...
Routes sit between the page and the browser's network stack. The handler runs before the network stack has processed the request: [Route.resume()](/api/class-route.mdx#route-continue) passes it on, [Route.fulfill()](/api/class-route.mdx#route-fulfill) answers it without touching the network, and [Route.abort()](/api/class-route.mdx#route-abort) fails it.
