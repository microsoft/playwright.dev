...
### Behavior Changes
- [assertThat(locator).hasAttribute(name, value[, options])](/test-assertions.mdx#locator-assertions-to-have-attribute) with an empty value does not match missing attribute anymore. For example, the following snippet will succeed when `button` **does not** have a `disabled` attribute.
...
* New method [assertThat(locator).hasValues(values[, options])](/test-assertions.mdx#locator-assertions-to-have-values) that asserts all selected values of `<select multiple>` element.
* Methods [assertThat(locator).containsText(expected[, options])](/test-assertions.mdx#locator-assertions-to-contain-text) and [assertThat(locator).hasText(expected[, options])](/test-assertions.mdx#locator-assertions-to-have-text) now accept `ignoreCase` option.
