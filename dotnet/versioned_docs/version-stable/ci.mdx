...
jobs:
  playwright:
    ...
    runs-on: ubuntu-latest
    container:
      image: mcr.microsoft.com/playwright/dotnet:v1.36.0-beta-1-jammy
    steps:
      - uses: actions/checkout@v3
      - name: Setup dotnet
        uses: actions/setup-dotnet@v3
...
pool:
  vmImage: ubuntu-latest
container: mcr.microsoft.com/playwright/dotnet:v1.36.0-beta-1-jammy
...
```groovy
pipeline {
   agent { docker { image 'mcr.microsoft.com/playwright/dotnet:v1.36.0-beta-1-jammy' } }
   stages {
      stage('e2e-tests') {
         steps {
            sh 'dotnet build'
