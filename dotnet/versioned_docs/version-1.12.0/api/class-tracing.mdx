...
- `options` &#60;`TracingStartOptions?`&#62;
  - `Name` &#60;[string]?&#62; If specified, the trace is going to be saved into the file with the given name inside the `tracesDir` folder specified in [BrowserType.LaunchAsync(options)](./api/class-browsertype.mdx#browser-type-launch).
