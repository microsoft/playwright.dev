...
Playwright Test is self-contained solution, it does not need Playwright [library](./library.mdx) to be installed. If you are an existing Playwright library user, make sure that you either uninstall it or update it prior to installing Playwright Test.

Unlike Playwright [library](./library.mdx), Playwright Test does not download browsers by default, so you need to install them explicitly:
