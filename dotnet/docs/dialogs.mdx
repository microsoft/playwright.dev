...
When [Page.CloseAsync()](/api/class-page.mdx#page-close) is invoked with the truthy `runBeforeUnload` value, the page runs its unload handlers. This is the only case when [Page.CloseAsync()](/api/class-page.mdx#page-close) does not wait for the page to actually close, because it might be that the page stays open in the end of the operation.
