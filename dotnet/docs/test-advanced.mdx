...
The port gets then passed over to Playwright as a `baseURL` when creating the context [Browser.NewContextAsync(options)](./api/class-browser.mdx#browser-new-context).
