...
Use [Expect(Locator).ToHaveTextAsync(expected, options)](/test-assertions.mdx#locator-assertions-to-have-text) to ensure that the list has the text "apple", "banana" and "orange".
...
The code inside [Locator.EvaluateAllAsync(expression, arg)](/api/class-locator.mdx#locator-evaluate-all) runs in the page, you can call any DOM apis there.
