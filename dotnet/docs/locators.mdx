...
Method [Locator.Filter()](/api/class-locator.mdx#locator-filter-2) narrows down an existing locator by matching an additional locator. For example, you can combine [Page.GetByRole()](/api/class-page.mdx#page-get-by-role) and [Page.GetByTitle()](/api/class-page.mdx#page-get-by-title) to match by both role and title.
