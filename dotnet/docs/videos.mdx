...
Playwright can record videos for all pages in a [browser context](./core-concepts.mdx#browser-contexts). Videos are saved upon context closure, so make sure to await [BrowserContext.CloseAsync()](./api/class-browsercontext.mdx#browser-context-close).
