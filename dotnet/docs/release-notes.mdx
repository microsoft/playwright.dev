...
### New APIs
- [Page.PdfAsync()](/api/class-page.mdx#page-pdf) accepts two new options `tagged` and `outline`.
...
- New options `style` in [Page.ScreenshotAsync()](/api/class-page.mdx#page-screenshot) and `style` in [Locator.ScreenshotAsync()](/api/class-locator.mdx#locator-screenshot) to add custom CSS to the page before taking a screenshot.
...
### New APIs
- Options `reason` in [Page.CloseAsync()](/api/class-page.mdx#page-close), `reason` in [BrowserContext.CloseAsync()](/api/class-browsercontext.mdx#browser-context-close) and `reason` in [Browser.CloseAsync()](/api/class-browser.mdx#browser-close). Close reason is reported for all operations interrupted by the closure.
- Option `firefoxUserPrefs` in [BrowserType.LaunchPersistentContextAsync()](/api/class-browsertype.mdx#browser-type-launch-persistent-context).
...
* Use new options `hasNot` and `hasNotText|hasNotTextRegex` in [Locator.Filter()](/api/class-locator.mdx#locator-filter) to find elements that **do not match** certain conditions.
...
- New option `hasNotText|hasNotTextRegex` in [Locator.Filter()](/api/class-locator.mdx#locator-filter)
- [Expect(Locator).ToBeAttachedAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-be-attached)
- New option `timeout` in [Route.FetchAsync()](/api/class-route.mdx#route-fetch)
...
- New option `maxRedirects` for method [Route.FetchAsync()](/api/class-route.mdx#route-fetch).
