...
- New option `style` in [Page.ScreenshotAsync()](/api/class-page.mdx#page-screenshot) and [Locator.ScreenshotAsync()](/api/class-locator.mdx#locator-screenshot) to add custom CSS to the page before taking a screenshot.
...
### New APIs
- Option `reason` in [Page.CloseAsync()](/api/class-page.mdx#page-close), [BrowserContext.CloseAsync()](/api/class-browsercontext.mdx#browser-context-close) and [Browser.CloseAsync()](/api/class-browser.mdx#browser-close). Close reason is reported for all operations interrupted by the closure.
