...
### Behavior Changes
- [Expect(Locator).ToHaveAttributeAsync();](/test-assertions.mdx#locator-assertions-to-have-attribute) with an empty value does not match missing attribute anymore. For example, the following snippet will succeed when `button` **does not** have a `disabled` attribute.
...
### Assertions
- New option `Enabled` for [Expect(Locator).ToBeEnabledAsync();](/test-assertions.mdx#locator-assertions-to-be-enabled).
- [Expect(Locator).ToHaveTextAsync();](/test-assertions.mdx#locator-assertions-to-have-text) now pierces open shadow roots.
- New option `Editable` for [Expect(Locator).ToBeEditableAsync();](/test-assertions.mdx#locator-assertions-to-be-editable).
- New option `Visible` for [Expect(Locator).ToBeVisibleAsync();](/test-assertions.mdx#locator-assertions-to-be-visible).
...
* New method [Expect(Locator).ToHaveValuesAsync();](/test-assertions.mdx#locator-assertions-to-have-values) that asserts all selected values of `<select multiple>` element.
* Methods [Expect(Locator).ToContainTextAsync();](/test-assertions.mdx#locator-assertions-to-contain-text) and [Expect(Locator).ToHaveTextAsync();](/test-assertions.mdx#locator-assertions-to-have-text) now accept `ignoreCase` option.
