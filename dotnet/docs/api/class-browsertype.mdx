...
- `options` <`BrowserTypeLaunchPersistentContextOptions`>
  ...
  - `ReducedMotion` <"reduce"|"no-preference"> Emulates `'prefers-reduced-motion'` media feature, supported values are `'reduce'`, `'no-preference'`. See [Page.EmulateMediaAsync(options)](./api/class-page.mdx#pageemulatemediaasyncoptions) for more details. Defaults to `'no-preference'`.
