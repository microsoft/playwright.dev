...
Note that `headers` option will apply to the fetched request as well as any redirects initiated by it. If you want to only apply `headers` to the original request, but not to redirects, look into [Route.ContinueAsync()](/api/class-route.mdx#route-continue) instead.
