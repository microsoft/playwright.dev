...
- [Tracing.ExportAsync(path)](./api/class-tracing.mdx#tracingexportasyncpath)
- [Tracing.StartAsync(…)](./api/class-tracing.mdx#tracingstartasync)
- [Tracing.StopAsync()](./api/class-tracing.mdx#tracingstopasync)

## Tracing.ExportAsync(path)
- `path` <[string]> File to save the trace into.
- returns: <[void]>

Export trace into the file with the given name. Should be called after the tracing has stopped.

## Tracing.StartAsync(…)
- `name` <[string]> If specified, the trace is going to be saved into the file with the given name inside the `traceDir` folder specified in [BrowserType.LaunchAsync(…)](./api/class-browsertype.mdx#browsertypelaunchasync).
