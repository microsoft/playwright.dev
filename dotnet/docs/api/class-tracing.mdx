...
- [Tracing.StartAsync(options)](./api/class-tracing.mdx#tracingstartasyncoptions)
- [Tracing.StopAsync(options)](./api/class-tracing.mdx#tracingstopasyncoptions)

## Tracing.StartAsync(options)
- `options` <`TracingStartOptions`>
  - `Name` <[string]> If specified, the trace is going to be saved into the file with the given name inside the `tracesDir` folder specified in [BrowserType.LaunchAsync(options)](./api/class-browsertype.mdx#browsertypelaunchasyncoptions).
