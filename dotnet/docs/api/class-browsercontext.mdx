...
- [event BrowserContext.Request](./api/class-browsercontext.mdx#eventbrowsercontextrequest)
- [event BrowserContext.RequestFailed](./api/class-browsercontext.mdx#eventbrowsercontextrequestfailed)
- [event BrowserContext.RequestFinished](./api/class-browsercontext.mdx#eventbrowsercontextrequestfinished)
- [event BrowserContext.Response](./api/class-browsercontext.mdx#eventbrowsercontextresponse)
...
Emitted when a request is issued from any pages created through this context. The [request] object is read-only. To only listen for requests from a particular page, use [event Page.Request](./api/class-page.mdx#eventpagerequest).

In order to intercept and mutate requests, see [BrowserContext.RouteAsync(url, handler)](./api/class-browsercontext.mdx#browsercontextrouteasyncurl-handler) or [Page.RouteAsync(url, handler)](./api/class-page.mdx#pagerouteasyncurl-handler).

## event BrowserContext.RequestFailed
- type: <[Request]>

Emitted when a request fails, for example by timing out. To only listen for failed requests from a particular page, use [event Page.RequestFailed](./api/class-page.mdx#eventpagerequestfailed).

:::note
HTTP Error responses, such as 404 or 503, are still successful responses from HTTP standpoint, so request will complete with [event BrowserContext.RequestFinished](./api/class-browsercontext.mdx#eventbrowsercontextrequestfinished) event and not with [event BrowserContext.RequestFailed](./api/class-browsercontext.mdx#eventbrowsercontextrequestfailed).
:::

## event BrowserContext.RequestFinished
- type: <[Request]>

Emitted when a request finishes successfully after downloading the response body. For a successful response, the sequence of events is `request`, `response` and `requestfinished`. To listen for successful requests from a particular page, use [event Page.RequestFinished](./api/class-page.mdx#eventpagerequestfinished).

## event BrowserContext.Response
- type: <[Response]>

Emitted when [response] status and headers are received for a request. For a successful response, the sequence of events is `request`, `response` and `requestfinished`. To listen for response events from a particular page, use [event Page.Response](./api/class-page.mdx#eventpageresponse).
