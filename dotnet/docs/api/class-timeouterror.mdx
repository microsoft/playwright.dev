...
TimeoutError is emitted whenever certain operations are terminated due to timeout, e.g. [Page.WaitForSelectorAsync(selector, options)](./api/class-page.mdx#pagewaitforselectorasyncselector-options) or [BrowserType.LaunchAsync(options)](./api/class-browsertype.mdx#browsertypelaunchasyncoptions).
