...
TimeoutError is emitted whenever certain operations are terminated due to timeout, e.g. [Page.WaitForSelectorAsync(selector, options)](./api/class-page.mdx#page-wait-for-selector) or [BrowserType.LaunchAsync(options)](./api/class-browsertype.mdx#browser-type-launch).
