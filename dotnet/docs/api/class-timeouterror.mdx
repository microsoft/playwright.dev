...
TimeoutError is emitted whenever certain operations are terminated due to timeout, e.g. [Page.WaitForSelectorAsync(selector, …)](./api/class-page.mdx#pagewaitforselectorasyncselector-) or [BrowserType.LaunchAsync(…)](./api/class-browsertype.mdx#browsertypelaunchasync).
