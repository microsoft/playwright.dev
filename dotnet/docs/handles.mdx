...
:::note
It is recommended to use selector-based actions like [Page.ClickAsync(selector, options)](./api/class-page.mdx#pageclickasyncselector-options) rather than using the [ElementHandle] for input actions, unless your use case specifically requires the use of handles.
:::

When [ElementHandle] is required, it is recommended to fetch it with the [Page.WaitForSelectorAsync(selector, options)](./api/class-page.mdx#pagewaitforselectorasyncselector-options) or [Frame.WaitForSelectorAsync(selector, options)](./api/class-frame.mdx#framewaitforselectorasyncselector-options) methods. These APIs wait for the element to be attached and visible.
