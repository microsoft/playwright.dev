...
When you need to target a parent element of some other element, most of the time you should [Locator.Filter()](/api/class-locator.mdx#locator-filter-1) by the child locator. For example, consider the following DOM structure:
...
If you'd like to target the parent `<li>` of a label with text `"Hello"`, using [Locator.Filter()](/api/class-locator.mdx#locator-filter-1) works best:
...
Alternatively, if you cannot find a suitable locator for the parent element, use `xpath=..`. Note that this method is not as reliable, because any changes to the DOM structure will break your tests. Prefer [Locator.Filter()](/api/class-locator.mdx#locator-filter-1) when possible.
...
If you'd like to target one of the two or more elements, and you don't know which one it will be, use [Locator.Or()](/api/class-locator.mdx#locator-or) to create a locator that matches any of the alternatives.
...
For example, consider the following DOM structure: `<label for="password">Password:</label><input id="password" type="password">`. You can target the label by it's "Password" text using [Page.GetByText()](/api/class-page.mdx#page-get-by-text). However, the following actions will be performed on the input instead of the label:
...
:::warning
We recommend [chaining locators](./locators.mdx#chaining-locators) instead.
:::
