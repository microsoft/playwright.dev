...
- **Console helper**: Configures a `playwright` object in the browser to generate and highlight [Playwright selectors](./locators.mdx#selectors). This can be used to verify text or composite selectors.
...
The Inspector opens up a browser window highlighting the selectors as you step through each line of the test. Use the explore button to find other available [selectors](./locators.mdx#selectors) which you can then copy into your test file and rerun your tests to see if they pass.
...
Using a [Page.PauseAsync()](/api/class-page.mdx#page-pause) method is an easy way to pause the Playwright script execution and inspect the page in Developer tools. It will also open Playwright Inspector to help with debugging.
