...
Playwright Tests supports browser and context options that you typically pass to [BrowserType.LaunchAsync(options)](./api/class-browsertype.mdx#browsertypelaunchasyncoptions) and [Browser.NewContextAsync(options)](./api/class-browser.mdx#browsernewcontextasyncoptions) methods, for example `headless`, `viewport` or `ignoreHTTPSErrors`. It also provides options to record video for the test or capture screenshot at the end.

You can specify any options either locally in a test file, or globally in the configuration file.
- Browser options match [BrowserType.LaunchAsync(options)](./api/class-browsertype.mdx#browsertypelaunchasyncoptions) method.
- Context options match [Browser.NewContextAsync(options)](./api/class-browser.mdx#browsernewcontextasyncoptions) method.
...
Alternatively, you can use [Page.RouteAsync(url, handler)](./api/class-page.mdx#pagerouteasyncurl-handler) to mock network in a single test.
