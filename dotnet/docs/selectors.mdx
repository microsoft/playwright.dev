...
Selectors are strings that are used to create [Locator]s. Locators are used to perform actions on the elements by means of methods such as [Locator.ClickAsync(options)](./api/class-locator.mdx#locator-click), [Locator.FillAsync(value, options)](./api/class-locator.mdx#locator-fill) and alike.
...
`:nth-match()` is also useful to wait until a specified number of elements appear, using [Locator.WaitForAsync(options)](./api/class-locator.mdx#locator-wait-for).
