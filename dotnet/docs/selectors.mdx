...
Selectors are strings that are used to create [Locator]s. Locators are used to perform actions on the elements by means of methods such as [Locator.ClickAsync();](/api/class-locator.mdx#locator-click), [Locator.FillAsync();](/api/class-locator.mdx#locator-fill) and alike. For debugging selectors, see [here](./debug-selectors).
