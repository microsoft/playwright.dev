...
Navigating to a URL auto-waits for the page to fire the `load` event. If the page does a client-side redirect before `load`, [Page.GotoAsync()](/api/class-page.mdx#page-goto) will auto-wait for the redirected page to fire the `load` event.
...
In lazy-loaded pages, it can be useful to wait until an element is visible with [Locator.WaitForAsync()](/api/class-locator.mdx#locator-wait-for). Alternatively, page interactions like [Page.ClickAsync()](/api/class-page.mdx#page-click) auto-wait for elements.
...
In the scenarios below, [Locator.ClickAsync()](/api/class-locator.mdx#locator-click) initiates a navigation and then waits for the navigation to complete.

### Auto-wait

By default, [Locator.ClickAsync()](/api/class-locator.mdx#locator-click) will wait for the navigation step to complete. This can be combined with a page interaction on the navigated page which would auto-wait for an element.
...
`locator.click` can be combined with [Page.WaitForLoadStateAsync()](/api/class-page.mdx#page-wait-for-load-state) to wait for a loading event.
...
In lazy-loaded pages, it can be useful to wait until an element is visible with [Locator.WaitForAsync()](/api/class-locator.mdx#locator-wait-for). Alternatively, page interactions like [Locator.ClickAsync()](/api/class-locator.mdx#locator-click) auto-wait for elements.
...
Clicking an element could trigger asynchronous processing before initiating the navigation. In these cases, it is recommended to explicitly call [Page.RunAndWaitForNavigationAsync()](/api/class-page.mdx#page-wait-for-navigation). For example:
...
Clicking an element could trigger multiple navigations. In these cases, it is recommended to explicitly [Page.RunAndWaitForNavigationAsync()](/api/class-page.mdx#page-wait-for-navigation) to a specific url. For example:
...
When popup is opened, explicitly calling [Page.WaitForLoadStateAsync()](/api/class-page.mdx#page-wait-for-load-state) ensures that popup is loaded to the desired state.
...
For pages that have complicated loading patterns, [Page.WaitForFunctionAsync()](/api/class-page.mdx#page-wait-for-function) is a powerful and extensible approach to define a custom wait criteria.
