...
Alternatively, instead of using the CLI, you can record HAR programmatically. Pass `har` option when creating a [BrowserContext] with [Browser.NewContextAsync(options)](./api/class-browser.mdx#browser-new-context) to create an archive.
...
Pass `har` option to the [Browser.NewContextAsync(options)](./api/class-browser.mdx#browser-new-context) method to use matching responses from the HAR file.
