...
```bash
docker pull mcr.microsoft.com/playwright/dotnet:v1.38.0-jammy
```
...
```bash
docker run -it --rm --ipc=host mcr.microsoft.com/playwright/dotnet:v1.38.0-jammy /bin/bash
```
...
```bash
docker run -it --rm --ipc=host --user pwuser --security-opt seccomp=seccomp_profile.json mcr.microsoft.com/playwright/dotnet:v1.38.0-jammy /bin/bash
```
