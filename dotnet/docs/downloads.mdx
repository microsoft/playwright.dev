...
For every attachment downloaded by the page, [Page.Download](/api/class-page.mdx#page-event-download) event is emitted. All these attachments are downloaded into a temporary folder. You can obtain the download url, file system path and payload stream using the [Download] object from the event.

You can specify where to persist downloaded files using the `downloadsPath` option in [BrowserType.LaunchAsync();](/api/class-browsertype.mdx#browser-type-launch).
