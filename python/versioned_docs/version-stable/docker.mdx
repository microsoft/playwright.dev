...
```bash
docker pull mcr.microsoft.com/playwright/python:v1.30.0-focal
```
...
```bash
docker run -it --rm --ipc=host mcr.microsoft.com/playwright/python:v1.30.0-focal /bin/bash
```
...
```bash
docker run -it --rm --ipc=host --user pwuser --security-opt seccomp=seccomp_profile.json mcr.microsoft.com/playwright/python:v1.30.0-focal /bin/bash
```
