...
```bash
docker run -it --rm --ipc=host mcr.microsoft.com/playwright/python:v1.47.0-jammy /bin/bash
```
...
```bash
docker run -it --rm --ipc=host --user pwuser --security-opt seccomp=seccomp_profile.json mcr.microsoft.com/playwright/python:v1.47.0-jammy /bin/bash
```
...
- `:v1.47.0` - Playwright v1.47.0 release docker image based on Ubuntu 22.04 LTS (Jammy Jellyfish).
