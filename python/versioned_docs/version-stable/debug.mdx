...
- **Console helper**: Configures a `playwright` object in the browser to generate and highlight [Playwright locators](./locators.mdx). This can be used to verify locators.
...
The Inspector opens up a browser window highlighting the elements as you step through each line of the test. Use the explore button to pick a [locator](./locators.mdx) which you can then copy into your test file and rerun your tests to see if they pass.
...
Playwright runs browsers in headless mode by default. To change this behavior, use `headless: false` as a launch option. You can also use the `slow_mo` option to slow down execution (by N milliseconds per operation) and follow along while debugging.
