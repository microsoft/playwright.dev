...
For example, for [page.click(selector, **kwargs)](/api/class-page.mdx#page-click), Playwright will ensure that:
...
Some actions like [page.click(selector, **kwargs)](/api/class-page.mdx#page-click) support `force` option that disables non-essential actionability checks, for example passing truthy `force` to [page.click(selector, **kwargs)](/api/class-page.mdx#page-click) method will not check that the target element actually receives click events.
...
For example, consider a scenario where Playwright will click `Sign Up` button regardless of when the [page.click(selector, **kwargs)](/api/class-page.mdx#page-click) call was made:
