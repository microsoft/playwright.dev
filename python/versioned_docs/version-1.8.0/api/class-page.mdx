...
Emitted when a JavaScript dialog appears, such as `alert`, `prompt`, `confirm` or `beforeunload`. Playwright can respond to the dialog via [dialog.accept(**kwargs)](./api/class-dialog.mdx#dialogacceptkwargs) or [dialog.dismiss()](./api/class-dialog.mdx#dialogdismiss) methods.
...
If `page_function` returns a [Promise], then [page.eval_on_selector(selector, expression, **kwargs)](./api/class-page.mdx#pageeval_on_selectorselector-expression-kwargs) would wait for the promise to resolve and return its value.
...
If `page_function` returns a [Promise], then [page.eval_on_selector_all(selector, expression, **kwargs)](./api/class-page.mdx#pageeval_on_selector_allselector-expression-kwargs) would wait for the promise to resolve and return its value.
...
If the function passed to the [page.evaluate(expression, **kwargs)](./api/class-page.mdx#pageevaluateexpression-kwargs) returns a non-[Serializable] value, then [page.evaluate(expression, **kwargs)](./api/class-page.mdx#pageevaluateexpression-kwargs) resolves to `undefined`. DevTools Protocol also supports transferring some additional values that are not serializable by `JSON`: `-0`, `NaN`, `Infinity`, `-Infinity`, and bigint literals.
