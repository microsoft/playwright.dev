...
The only difference between [frame.evaluate(expression, **options)](./api/class-frame.mdx#frameevaluateexpression-options) and [frame.evaluate_handle(expression, **options)](./api/class-frame.mdx#frameevaluate_handleexpression-options) is that [method: Frame.evaluateHandle`] returns in-page object (JSHandle).

If the function, passed to the [frame.evaluate_handle(expression, **options)](./api/class-frame.mdx#frameevaluate_handleexpression-options), returns a [Promise], then [frame.evaluate_handle(expression, **options)](./api/class-frame.mdx#frameevaluate_handleexpression-options) would wait for the promise to resolve and return its value.
...
The [frame.wait_for_function(expression, **options)](./api/class-frame.mdx#framewait_for_functionexpression-options) can be used to observe viewport size change:
