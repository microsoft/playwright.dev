...
1. Run tests (for example, with `pytest`). 2. Login via UI and retrieve authentication state. 3. In each test, load authentication state using `autouse=True` fixture with `scope=function`.
