...
By default, Playwright runs the browsers in headless mode. To see the browser UI, set `headless` option to `False`. You can also use `slow_mo` to slow down execution. Learn more in the debugging tools [section](./debug.mdx).
