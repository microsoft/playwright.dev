...
* **Modern web features**. Playwright supports web components through [shadow-piercing selectors](./locators.mdx#selectors), geolocation, permissions, web workers and other modern web APIs.
- **Capabilities to cover all scenarios**. Support for [file downloads](./downloads.mdx) and [uploads](./input.mdx), out-of-process iframes, native [input events](./input.mdx), and even dark mode.
