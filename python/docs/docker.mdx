...
```bash
docker pull mcr.microsoft.com/playwright/python:v1.47.2-noble
```
...
```bash
docker run -it --rm --ipc=host mcr.microsoft.com/playwright/python:v1.47.2-noble /bin/bash
```
...
```bash
docker run -it --rm --ipc=host --user pwuser --security-opt seccomp=seccomp_profile.json mcr.microsoft.com/playwright/python:v1.47.2-noble /bin/bash
```
...
RUN pip install playwright==@1.47.2 && \
