...
### Behavior Changes
- [expect(locator).to_have_attribute()](/test-assertions.mdx#locator-assertions-to-have-attribute) with an empty value does not match missing attribute anymore. For example, the following snippet will succeed when `button` **does not** have a `disabled` attribute.
...
### Assertions
- New option `enabled` for [expect(locator).to_be_enabled()](/test-assertions.mdx#locator-assertions-to-be-enabled).
- [expect(locator).to_have_text()](/test-assertions.mdx#locator-assertions-to-have-text) now pierces open shadow roots.
- New option `editable` for [expect(locator).to_be_editable()](/test-assertions.mdx#locator-assertions-to-be-editable).
- New option `visible` for [expect(locator).to_be_visible()](/test-assertions.mdx#locator-assertions-to-be-visible).
...
* New method [expect(locator).to_have_values()](/test-assertions.mdx#locator-assertions-to-have-values) that asserts all selected values of `<select multiple>` element.
* Methods [expect(locator).to_contain_text()](/test-assertions.mdx#locator-assertions-to-contain-text) and [expect(locator).to_have_text()](/test-assertions.mdx#locator-assertions-to-have-text) now accept `ignore_case` option.
