...
Playwright Tests supports browser and context options that you typically pass to [browser_type.launch(**kwargs)](./api/class-browsertype.mdx#browser_typelaunchkwargs) and [browser.new_context(**kwargs)](./api/class-browser.mdx#browsernew_contextkwargs) methods, for example `headless`, `viewport` or `ignoreHTTPSErrors`. It also provides options to record video for the test or capture screenshot at the end.

You can specify any options either locally in a test file, or globally in the configuration file.
- Browser options match [browser_type.launch(**kwargs)](./api/class-browsertype.mdx#browser_typelaunchkwargs) method.
- Context options match [browser.new_context(**kwargs)](./api/class-browser.mdx#browsernew_contextkwargs) method.
