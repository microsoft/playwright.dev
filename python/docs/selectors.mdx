...
Selectors query elements on the web page for interactions, like [page.click(selector, **options)](./api/class-page.mdx#pageclickselector-options), and to obtain `ElementHandle` through [page.query_selector(selector)](./api/class-page.mdx#pagequery_selectorselector). Built-in selectors auto-pierce [shadow DOM](https://developer.mozilla.org/en-US/docs/Web/Web_Components/Using_shadow_DOM).
...
Selector describes an element in the page. It can be used to obtain `ElementHandle` (see [page.query_selector(selector)](./api/class-page.mdx#pagequery_selectorselector) for example) or shortcut element operations to avoid intermediate handle (see [page.click(selector, **options)](./api/class-page.mdx#pageclickselector-options) for example).
...
* [Custom selector engines](./extensibility.mdx)
...
The `:visible` pseudo-class matches elements that are visible as defined in the [actionability](./actionability.mdx#visible) guide. For example, `input` matches all the inputs on the page, while `input:visible` matches only visible inputs. This is useful to distinguish elements that are very similar but differ in visibility, however it's usually better to follow [best practices](#best-practices) and find another way to select the element.
