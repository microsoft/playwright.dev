...
Selectors are strings that are used to create [Locator]s. Locators are used to perform actions on the elements by means of methods such as [locator.click(**kwargs)](./api/class-locator.mdx#locator-click), [locator.fill(value, **kwargs)](./api/class-locator.mdx#locator-fill) and alike.
...
- [Augmenting existing locators](#augmenting-existing-locators)
...
- [Selecting elements by label text](#selecting-elements-by-label-text)
...
Note that resulting matches are sorted by their distance to the anchor element, so you can use [locator.first](./api/class-locator.mdx#locator-first) to pick the closest one.
