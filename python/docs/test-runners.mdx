...
- `connect_options`: Connect to an existing browser via WebSocket endpoint. It should return a Dict with [browser_type.connect()](/api/class-browsertype.mdx#browser-type-connect) options.
