...
- `context`: New [browser context](./browser-contexts.mdx) for a test.
- `page`: New [browser page](./pages.mdx) for a test.
- `new_context`: Allows creating different [browser contexts](./browser-contexts.mdx) for a test. Useful for multi-user scenarios. Accepts the same parameters as [browser.new_context()](/api/class-browser.mdx#browser-new-context).
