...
- [tracing.start_chunk()](./api/class-tracing.mdx#tracing-start-chunk)
...
## tracing.start_chunk() {#tracing-start-chunk}
...
Start a new trace chunk. If you'd like to record multiple traces on the same [BrowserContext], use [tracing.start(**kwargs)](./api/class-tracing.mdx#tracing-start) once, and then create multiple trace chunks with [tracing.start_chunk()](./api/class-tracing.mdx#tracing-start-chunk) and [tracing.stop_chunk(**kwargs)](./api/class-tracing.mdx#tracing-stop-chunk).
...
- `path`<a aria-hidden="true" tabindex="-1" class="list-anchor-link" id="tracing-stop-chunk-option-path"/> &#60;[Union]\[[str], [pathlib.Path]\]&#62; Export trace collected since the last [tracing.start_chunk()](./api/class-tracing.mdx#tracing-start-chunk) call into the file with the given path.<a href="#tracing-stop-chunk-option-path" class="list-anchor">#</a>
