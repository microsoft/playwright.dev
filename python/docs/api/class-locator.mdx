...
Captures the aria snapshot of the given element. Read more about [accessibility snapshots](../aria-snapshot.mdx) and [expect(locator).to_match_aria_snapshot()](/api/class-locatorassertions.mdx#locator-assertions-to-match-aria-snapshot) for the corresponding assertion.
...
- `mask` [List]\[[Locator]\] *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="locator-screenshot-option-mask"/><a href="#locator-screenshot-option-mask" class="list-anchor">#</a>
  ...
  Specify locators that should be masked when the screenshot is taken. Masked elements will be overlaid with a pink box `#FF00FF` (customized by [mask_color](/api/class-locator.mdx#locator-screenshot-option-mask-color)) that completely covers its bounding box.
