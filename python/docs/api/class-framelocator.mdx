...
If you have a [Locator] object pointing to an `iframe` it can be converted to [FrameLocator] using [locator.enter_frame](/api/class-locator.mdx#locator-enter-frame).

**Converting FrameLocator to Locator**

If you have a [FrameLocator] object it can be converted to [Locator] pointing to the same `iframe` using [frame_locator.exit_frame](/api/class-framelocator.mdx#frame-locator-exit-frame).
...
### exit_frame {#frame-locator-exit-frame}

<font size="2" style={{position: "relative", top: "-20px"}}>Added in: v1.43</font><x-search>frameLocator.exit_frame</x-search>
