...
Returns the return value of `expression` as in-page object (JSHandle).

The only difference between [frame.evaluate(expression, **kwargs)](./api/class-frame.mdx#frameevaluateexpression-kwargs) and [frame.evaluate_handle(expression, **kwargs)](./api/class-frame.mdx#frameevaluate_handleexpression-kwargs) is that [method: Frame.evaluateHandle`] returns in-page object (JSHandle).
