...
- `no_wait_for_finish` [bool] *(optional)* <font size="2">Added in: v1.41</font><a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="browser-context-route-option-no-wait-for-finish"/><a href="#browser-context-route-option-no-wait-for-finish" class="list-anchor">#</a>
  
  If set to true, [browser_context.close()](/api/class-browsercontext.mdx#browser-context-close) and [page.close()](/api/class-page.mdx#page-close) will not wait for the handler to finish and all errors thrown by then handler after the context has been closed are silently caught. Defaults to false.
...
- `no_wait_for_active` [bool] *(optional)* <font size="2">Added in: v1.41</font><a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="browser-context-unroute-option-no-wait-for-active"/><a href="#browser-context-unroute-option-no-wait-for-active" class="list-anchor">#</a>
  
  If set to true, [browser_context.unroute()](/api/class-browsercontext.mdx#browser-context-unroute) will not wait for current handler call (if any) to finish and all errors thrown by the handler after unrouting are silently caught. Defaults to false.
