...
If the function passed to the [page.evaluate(expression, **options)](./api/class-page.mdx#pageevaluateexpression-options) returns a non-[Serializable] value, then [page.evaluate(expression, **options)](./api/class-page.mdx#pageevaluateexpression-options) resolves to `undefined`. DevTools Protocol also supports transferring some additional values that are not serializable by `JSON`: `-0`, `NaN`, `Infinity`, `-Infinity`, and bigint literals.
...
:::note
In most cases, you should use [page.expect_event(event, **options)](./api/class-page.mdx#pageexpect_eventevent-options).
:::
...
The [page.wait_for_function(expression, **options)](./api/class-page.mdx#pagewait_for_functionexpression-options) can be used to observe viewport size change:
