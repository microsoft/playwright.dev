...
<font size="2" style={{position: "relative", top: "-20px"}}>Added in: v1.59</font><x-search>page.overlay</x-search>
...
Video object associated with this page. Can be used to control video recording with [video.start()](/api/class-video.mdx#video-start) and [video.stop()](/api/class-video.mdx#video-stop), or to access the video file when using the `recordVideo` context option.
