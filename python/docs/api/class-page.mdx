...
* After executing the handler, Playwright will ensure that overlay that triggered the handler is not visible anymore. You can opt-out of this behavior with `allow_staying_visible`.
...
An example with a custom callback on every actionability check. It uses a `<body>` locator that is always visible, so the handler is called before every actionability check. It is important to specify `allow_staying_visible`, because the handler does not hide the `<body>` element.
...
- `allow_staying_visible` [bool] *(optional)* <font size="2">Added in: v1.44</font><a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="page-add-locator-handler-option-allow-staying-visible"/><a href="#page-add-locator-handler-option-allow-staying-visible" class="list-anchor">#</a>
...
Removes locator handler added by [page.add_locator_handler()](/api/class-page.mdx#page-add-locator-handler).

**Usage**

```python
page.remove_locator_handler(locator, handler)
```
...
- `handler` [Callable]\[[Locator]\]:[Promise]\[[Any]\]<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="page-remove-locator-handler-option-handler"/><a href="#page-remove-locator-handler-option-handler" class="list-anchor">#</a>
  
  Handler passed to [page.add_locator_handler()](/api/class-page.mdx#page-add-locator-handler).
