...
Each version of Playwright needs specific versions of browser binaries to operate. Depending on the language you use, Playwright will either download these browsers at package install time for you or will require you to use [Playwright CLI](./cli.mdx) to install the browsers. Please refer to the [getting started](./intro.mdx) to see what your platform port does.
...
:::note
Playwright bundles a recent Chromium build, but not Google Chrome or Microsoft Edge browsers - these should be installed manually before use. You can use Playwright [Command Line Interface](./cli.mdx#install-browsers) to install the browsers.
:::
