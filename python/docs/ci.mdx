...
jobs:
  playwright:
    ...
    runs-on: ubuntu-latest
    container:
      image: mcr.microsoft.com/playwright/python:v1.47.2-jammy
      options: --user 1001
    steps:
      - uses: actions/checkout@v4
      - name: Set up Python
...
pool:
  vmImage: ubuntu-latest
container: mcr.microsoft.com/playwright/python:v1.47.2-noble
...
executors:
  pw-jammy-development:
    docker:
      - image: mcr.microsoft.com/playwright/python:v1.47.2-noble
```
...
```groovy
pipeline {
   agent { docker { image 'mcr.microsoft.com/playwright/python:v1.47.2-noble' } }
   stages {
      stage('e2e-tests') {
         steps {
