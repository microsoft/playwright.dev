...
If you would like to use codegen in some non-standard setup (for example, use [browser_context.route(url, handler)](./api/class-browsercontext.mdx#browser-context-route)), it is possible to call [page.pause()](./api/class-page.mdx#page-pause) that will open a separate window with codegen controls.
