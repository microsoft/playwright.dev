...
- [page.get_by_role(role, **kwargs)](#locate-based-on-accessible-attributes) to locate by explicit and implicit accessibility attributes.
- [page.get_by_text(text, **kwargs)](#locate-by-text) to locate by text content.
- [page.get_by_label(text, **kwargs)](#locate-by-label-text) to locate a form control by associated label's text.
- [page.get_by_placeholder(text, **kwargs)](#locate-by-placeholder-text) to locate an input by placeholder.
- [page.get_by_alt_text(text, **kwargs)](#locate-by-alt-text) to locate an element, usually image, by its text alternative.
- [page.get_by_title(text, **kwargs)](#locate-by-title) to locate an element by its title.
- [page.get_by_test_id(test_id)](#define-explicit-contract-and-use-a-data-testid-attribute) to locate an element based on its `data-testid` attribute (other attribute can be configured).
