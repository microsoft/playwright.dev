...
Locators can be filtered by text with the [locator.filter()](/api/class-locator.mdx#locator-filter-1) method. It will search for a particular string somewhere inside the element, possibly in a descendant element, case-insensitively. You can also pass a regular expression.
...
Method [locator.filter()](/api/class-locator.mdx#locator-filter-2) narrows down an existing locator by matching an additional locator. For example, you can combine [page.get_by_role()](/api/class-page.mdx#page-get-by-role) and [page.get_by_title()](/api/class-page.mdx#page-get-by-title) to match by both role and title.
...
Use the [page.get_by_text()](/api/class-page.mdx#page-get-by-text) method to locate an element in a list by it's text content and then click on it.
...
Use the [locator.filter()](/api/class-locator.mdx#locator-filter-1) to locate a specific item in a list.
...
When you have elements with various similarities, you can use the [locator.filter()](/api/class-locator.mdx#locator-filter-1) method to select the right one. You can also chain multiple filters to narrow down the selection.
