...
- [Request.GetPayloadAsJson([options])](./api/class-request.mdx#requestgetpayloadasjsonoptions)
...
## Request.GetPayloadAsJson([options])
- `options` <`Request.GetPayloadAsJsonOptions`>
  - `documentOptions` <[null]|[JsonDocumentOptions]> The options that control custom behaviour when parsing the JSON.
- returns: <[JsonDocument]>

Returns a [JsonDocument] representation of [Request.PostDataBuffer()](./api/class-request.mdx#requestpostdatabuffer).
