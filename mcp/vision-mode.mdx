...
| `browser_mouse_click_xy` | Click at x,y (supports button, clickCount, delay) |
...
See [Keyboard & Mouse](./tools/keyboard-mouse.mdx) for the full parameter reference.
...
:::tip
Instead of reading coordinates off a screenshot, ask for them directly:
`browser_snapshot { boxes: true }` adds `[box=x,y,width,height]` to every node, viewport-relative in
CSS pixels. `--snapshot-boxes` turns this on for every snapshot.
:::
